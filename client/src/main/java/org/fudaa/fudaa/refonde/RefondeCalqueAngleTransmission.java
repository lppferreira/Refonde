/*
 * @file         RefondeCalqueAngleTransmission.java
 * @creation     1999-08-10
...
/**
 * Un calque de tracé des angles de transmission pour les digues transmissibles.
 *
 * @version      $Id: RefondeCalqueAngleTransmission.java,v 1.7 2006-09-19 15:10:23 deniger Exp $
 * @author       Bertrand Marchand , Bertrand Marchand
 */
public class RefondeCalqueAngleTransmission extends BCalqueAffichage {
   ...
   */
  public RefondeCalqueAngleTransmission() {
    super();
    reinitialise();
  ...
  public void initialise(RefondeProjet _projet) {
    ...
    reinitialise();
    //--------------------------------------------------------------------------
    // Angles de transmission sur les domaines digues
    //--------------------------------------------------------------------------
    aisDigue= projet_.getModeleCalcul().getAnglesTransmissionDigues();
    //    for (int i=0; i<aisDigue.length; i++) {
    //      pl=projet_.getModeleCalcul().getPolyligneAT(aisDigue[i]);
    //      ajouteAngle(aisDigue[i],pl);
    //    }
    // B.M. 06/09/2000 Orientation des digues de manière à ce que les repères
    // des angles de transmission soient toujours orientés vers l'extérieur de
    // la digue
    ...
    for (int i= 0; i < dms.size(); i++) {
      ...
      ctOr.orienteTrigo(true);
      for (int j= 0; j < aisDigue.length; j++) {
        pl= projet_.getModeleCalcul().getPolyligneAT(aisDigue[j]);
