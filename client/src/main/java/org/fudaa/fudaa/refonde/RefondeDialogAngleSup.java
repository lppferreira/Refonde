/*
 * @file         RefondeDialogAngleSup.java
 * @creation     2001-12-03
...
/**
 * Une boite de dialogue permettant de modifier un angle supplémentaire.
 *
 * @version      $Id: RefondeDialogAngleSup.java,v 1.9 2007-01-19 13:14:14 deniger Exp $
...
public class RefondeDialogAngleSup extends FudaaDialog {
  ...
  public RefondeDialogAngleSup() {
    this(null);
  }
  public RefondeDialogAngleSup(Frame _parent) {
  ...
  public void jbInit() { /**JBuilder**/
    ...
    setModal(true);
    setTitle("Définition de l'angle supplémentaire");
    pack();
  ...
  public void initialise(RefondeProjet _projet, Object[] _selObjs) {
    ...
    tpAng= ((RefondeAngle)selObjs_[0]).getTypeSup();
    for (int i= 1; i < selObjs_.length; i++) {
      if (tpAng != ((RefondeAngle)selObjs_[i]).getTypeSup()) {
        tpAng= -1; // Type mixte.
        break;
      }
    }
    ...
    if (tpAng != -1) {
      for (int i= 0; i < selObjs_.length; i++) {
        ...
        //            pyDiff=p.y;
        //          }
        //          else if (pxDiff!=p.x || pyDiff!=p.y) {
        //            pxDiff=Double.NaN;
        //            pyDiff=Double.NaN;
        //            break;
        //          }
        //        }
        if (tpAng == RefondeAngle.RELATIF) {
          double angle= ((RefondeAngle)selObjs_[i]).getAngleSup();
          if (i == 0) {
            angRel= angle;
          ...
          double angle=
            ((- ((RefondeAngle)selObjs_[i]).getAngleSup() + 270) % 360 + 360)
              % 360;
          if (i == 0) {
            angAbs= angle;
...
/*    double pxDiff= 0;
    ...
    for (int i= 0; i < selObjs_.length; i++) {
      ...
      //      }
      if (tpAng == RefondeAngle.RELATIF) {
        ai.setSupRelatif(angRel);
      } else if (tpAng == RefondeAngle.ABSOLU) {
        ai.setSupAbsolu(angAbs);
      }
    }
