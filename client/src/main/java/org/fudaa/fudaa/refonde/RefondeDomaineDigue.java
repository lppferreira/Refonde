...
/**
 * Domaine géometrique de type digue. Ce domaine ne contient qu'un seul contour,
 * peut être maillé en T3 ou Q4 suivant un mailleur qui lui est propre.
 *
...
public class RefondeDomaineDigue extends RefondeDomaine {
  ...
  protected int typeMaillage_= OUVRAGE; // Type de maillage
  protected GrPolyligne extDigue_;
  // Une des polylignes extremité de digue dans le contour
  protected int[] nbElements_;
  // nombre d'éléments sur chaque tronçon de la digue
  /**
  ...
  /**
   * Création d'un domaine de type digue
   */
  public RefondeDomaineDigue(RefondeContour _cntr) {
    this(_cntr, _cntr.getPolylignes()[0]);
  }
  /**
   * Création d'un domaine de type digue, en précisant l'extrémité de digue
   */
  public RefondeDomaineDigue(RefondeContour _cntr, RefondePolyligne _poly) {
    ...
    for (int i= 0; i < nbElements_.length; i++)
      nbElements_[i]= 3;
    setGroupeProprietes(
      new RefondeGroupeProprietes(
        RefondeGroupeProprietes.HOULE_FOND_PAROI_PERFOREE));
  ...
  /**
   * Nombre d'éléments par troncon, ordonnés à partir de l'extremité de digue
   */
  ...
  public void setContour(RefondeContour _cntr) {
    // Controle que la digue a un nombre pair de polylignes
    if (_cntr.getPolylignes().length % 2 != 0)
      throw new IllegalArgumentException("Le contour n'a pas un nombre pair de polylignes");
      ...
      for (int i= 0; i < pls.length; i++) {
        if (pls[i].hasNoeuds())
          throw new IllegalArgumentException(
            "Impossible de mailler le domaine\n"
              + "une des polylignes du contour a déjà des noeuds");
      }
      r= maillerQ4();
    }
    // Cote Z des noeuds ainsi créés
    nds= r.noeuds();
  ...
  protected GrMaillageElement maillerQ4() {
    ...
    for (int i= 0; i < plsOrigin.length; i++)
      if (plsOrigin[i] == extDigue_) {
        extOriente= pls[i];
        break;
      }
    ...
    pls= ctOriente.getPolylignes();
    int indiceExt;
    for (indiceExt= 0; indiceExt < pls.length; indiceExt++)
      if (pls[indiceExt] == extOriente)
        break;
    //    GrPolyligne[] pls=contours_[0].getPolylignes();
    //    int indiceExt=0;
    //    for (int i=0; i<pls.length; i++)
    //     if (pls[i]==extDigue_) { indiceExt=i; break; }
    ...
    // Détermination des points des troncons rectangulaires
    for (int i= 0; i < nbTroncons; i++) {
      ...
      for (int j= 0; j < 2; j++) {
        ...
        // Calcul des 4 intersections possibles
        for (int k= 0; k < 2; k++)
          pi[j * 2 + k]= dNorm[j * 2 + k].intersectionSegmentXY(sg[j]);
      }
      // Pb de géométrie si on n'a pas trouvé d'intersection
      if (pi[0] == null && pi[3] == null || pi[1] == null && pi[2] == null)
        throw new IllegalArgumentException("Géometrie mal définie");
      // Définition du troncon rectangulaire
      if (pi[0] == null) {
        pRect[i][0]= pi[3];
        pRect[i][3]= pTrcn[i][3];
      } else {
    ...
    // Construction des noeuds ordonnés
    //
    //                   Troncon 1
    //                2   4   6       8
    //                +---+---+------+
    ...
    for (int i= 0; i < nbTroncons; i++) {
      ...
      for (int j= 0; j < nbNoeuds; j++) {
        if (j == 0
          && (i == 0
            || pRect[i][0].distanceXY(pRect[i - 1][1]) < 1.e-4
            || pRect[i][3].distanceXY(pRect[i - 1][2]) < 1.e-4)) {
          ...
          j != nbNoeuds - 1
            || (pRect[i][1].distanceXY(pRect[i + 1][0]) > 1.e-4
              && pRect[i][2].distanceXY(pRect[i + 1][3]) > 1.e-4)) {
          vnds.add(
            nd=
              new GrNoeud(
                pRect[i][0].x_
                  + (pRect[i][1].x_ - pRect[i][0].x_) * j / (nbNoeuds - 1),
                pRect[i][0].y_
                  + (pRect[i][1].y_ - pRect[i][0].y_) * j / (nbNoeuds - 1),
                0.));
          nd.data(new RefondeNoeudData());
          vnds.add(
            nd=
              new GrNoeud(
                pRect[i][3].x_
                  + (pRect[i][2].x_ - pRect[i][3].x_) * j / (nbNoeuds - 1),
                pRect[i][3].y_
                  + (pRect[i][2].y_ - pRect[i][3].y_) * j / (nbNoeuds - 1),
                0.));
          nd.data(new RefondeNoeudData());
        }
      }
