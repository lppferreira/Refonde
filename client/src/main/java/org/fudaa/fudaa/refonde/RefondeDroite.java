...
/**
 * Classe représentant une droite. Pour l'instant locale, la classe ne fait pas
...
class RefondeDroite {
  ...
  /**
   * Création à partir d'un point et un vecteur
   */
  ...
  public GrPoint intersectionDroiteXY(RefondeDroite _d) {
    ...
    double yi;
    double prod;
    // Point d'intersection droite/droite
    a1= -vd1.y_;
    b1= vd1.x_;
    ...
    // Segment et droite parallèles => Pas de point d'intersection
    prod= a2 * b1 - a1 * b2;
    if (prod == 0.)
      return null;
    yi= (a1 * c2 - a2 * c1) / prod;
  ...
  public GrPoint intersectionSegmentXY(GrSegment _s) {
    ...
    if (r == null)
      return null;
    // Point d'intersection hors segment
    if (r.x_ + Math.abs(r.x_ * 1.e-10) < Math.min(_s.e_.x_, _s.o_.x_)
      || r.x_ - Math.abs(r.x_ * 1.e-10) > Math.max(_s.e_.x_, _s.o_.x_)
      || r.y_ + Math.abs(r.y_ * 1.e-10) < Math.min(_s.e_.y_, _s.o_.y_)
      || r.y_ - Math.abs(r.y_ * 1.e-10) > Math.max(_s.e_.y_, _s.o_.y_))
      return null;
    return r;
  }
}
